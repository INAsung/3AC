...
float central_threshold[6] = { 50, 50, 50, 80, 50, 50 };
float lateral_threshold[2] = { 100, 40 };
...
const float valveopen = 0.08 * 1e3;
...
// 2. lateral motors start to go forward, motor on for lateral_ex
// 3. give water when a lick is detected. valve open for valveopen
// * to prevent bias, water is not given for dominant choice (>70% choice or more than 5 consecutive choice)
// * if the mouse does not stick to one spout within a trial, lateral motors go back immediately.
// 4. lateral motors go back, motor on for central_in
...
void loop() {
  ...
  switch (status) {
    ...
    case 0:
      {  //#0 wait 5s to start
        if (now_time - status_start_time > ready2go) {
          Serial.println("stage 2 now start");
          status += 1;
    ...
    case 3:
      {  //#3-1 delivery sampling
        ...
        if (lateral_lick_status[0].lickstarted) {  // made decision! (lateral 1)
          if (trial_count - no_decision_count > 10 && ((float)lateral1_count / ((float)trial_count - (float)no_decision_count) > 0.7 || (switch_count > 4 && decision_prev == 1))) {
            status += 4;
          } else {
            status += 1;
          }
          ...
          if (trial_count - no_decision_count > 10 && ((float)lateral2_count / ((float)trial_count - (float)no_decision_count) > 0.7 || (switch_count > 4 && decision_prev == 2))) {
            status += 4;
          } else {
            status += 1;
          }
    ...
    case 4:
      {
        if ((decision == 1 && lateral_lick_status[1].lickstarted) || (decision == 2 && lateral_lick_status[0].lickstarted)) {
          status += 3;
    ...
    case 5:
      {  //#3-2 delivery to delivery offset
        if ((decision == 1 && lateral_lick_status[1].lickstarted) || (decision == 2 && lateral_lick_status[0].lickstarted)) {
          status += 2;
          digitalWrite(lateral_valve1, LOW);
          digitalWrite(lateral_valve2, LOW);
    ...
    case 8:
      {
        if (now_time - status_start_time > ITI - time_consumed) {
          ...
          Serial.println(String("trial # " + String(trial_count) + "ended. L1: " + String(lateral1_count) + ", L2: " + String(lateral2_count)) + ", no decision: " + String(no_decision_count)+ ", switching count: " + String(switch_count));
          trial_start_time = now_time;
          if (decision!=0){
